...
- [x] Follow-up questions work correctly — the LLM can reference prior messages in the session.
- [x] Conversations exceeding the context budget are automatically summarised without user intervention.
- [x] The rolling summary preserves key facts, decisions, and context from earlier in the conversation.
- [x] A long conversation (20+ turns) remains coherent and the LLM can reference early topics via the summary.
- [x] Context budget, recent window size, and summary prompt are configurable.
- [x] Performance is acceptable — summarisation does not add excessive latency to each turn (consider doing it asynchronously or only when the budget is exceeded).
- [x] Existing single-turn behaviour still works if no session context is provided.
