...
- [ ] The upload UI accepts multiple `.json` files in a single selection.
- [ ] All selected files are uploaded and processed without requiring the user to repeat the flow.
- [ ] Progress is visible for each file.
- [ ] Conversations that already exist in the database are skipped gracefully (no duplicates, no errors).
- [ ] A failure processing one file does not prevent the other files from being processed.
- [ ] A summary screen shows totals across all uploaded files.
- [ ] Existing single-file upload behaviour continues to work unchanged.
